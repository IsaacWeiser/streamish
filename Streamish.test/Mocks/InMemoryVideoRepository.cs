...
namespace Streamish.Tests.Mocks
{
    class InMemoryVideoRepository : IVideoRepository
    {
        ...
        public List<Video> Search(string criterion, bool sortDescending)
        {
            ...
            {
               return _data.Where(v => v.Title.Contains(criterion)).ToList().OrderByDescending(v => v.DateCreated).ToList();
            }
            else
            {
                return _data.Where(v => v.Title.Contains(criterion)).ToList().OrderBy(v=> v.DateCreated).ToList();
            }
        }
