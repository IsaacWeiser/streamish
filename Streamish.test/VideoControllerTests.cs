...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        [Fact]
        private void Search_Finds_Videos_Like_Criteria()
        {
            //arrange
            
            var vids = CreateTestVideos(30);

            var repo = new InMemoryVideoRepository(vids);
            var controller = new VideoController(repo);
            //act
            var result = controller.Search("q", true);
            //assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            Assert.IsType<List<Video>>(okResult.Value);
        }
    }
}