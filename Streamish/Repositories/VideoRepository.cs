    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Title as 'title', v.Description as 'description', v.Url as 'url', v.DateCreated as 'dateCreated', v.UserProfileId as 'userProfileId',
        ...
        public Video GetVideoByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @" SELECT v.Id as 'Id', v.Title as 'title', v.Description as 'description', v.Url as 'url', v.DateCreated as 'dateCreated', v.UserProfileId as 'userProfileId',
                            up.Id as 'upId', up.Name as 'upName', up.Email as 'email', up.ImageUrl as 'imgUrl', up.DateCreated as 'dateCreated',
        ...
        {
            var dateRequested = Convert.ToDateTime(date);
            using (var conn = Connection)
            {
                conn.Open();
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    FROM Video v 
                     JOIN UserProfile up ON v.UserProfileId = up.Id
                     WHERE v.DateCreated >= @dateRequested ";
